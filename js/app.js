...
function openDatabase(){return new Promise((resolve,reject)=>{const req=indexedDB.open('daftar-local-accounting',2);req.onupgradeneeded=()=>{const database=req.result;['records','history','settings','debts'].forEach(name=>{if(!database.objectStoreNames.contains(name))database.createObjectStore(name,{keyPath:'id'});});};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error);req.onblocked=()=>reject(new Error('أغلق علامات التبويب الأخرى ثم أعد المحاولة.'));});}
function getAll(store){return new Promise((resolve,reject)=>{const req=db.transaction(store).objectStore(store).getAll();req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error);});}
// Every record change and its history are committed in the same transaction.
function commit(changes){return new Promise((resolve,reject)=>{const tx=db.transaction(Object.keys(changes),'readwrite');tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error || new Error('تعذر حفظ البيانات'));for(const [store,items] of Object.entries(changes)){for(const item of items)tx.objectStore(store).put(item);}});}
async function refreshData(){const [r,h,s,d]=await Promise.all([getAll('records'),getAll('history'),getAll('settings'),getAll('debts')]);records=r;history=h;debts=d;settings=s.find(v=>v.id==='main')||structuredClone(defaultSettings);$('#brand-name').textContent=settings.name;document.title=`${settings.name} | المحاسبة المحلية`;}
...
async function saveEntry(event){
    ...
    try{
        ...
        for(const value of populated){
            ...
            const record={...(prior||{}),id:prior?.id||uid(),type,title,amount:Math.round(value.amount*100)/100,currency:value.currency,date,category,notes,photos:type==='expense'?[...(prior?.photos||[]),...pendingPhotos]:(prior?.photos||[]),createdAt:prior?.createdAt||now,updatedAt:now};
...
function validatePhoto(p){return p&&typeof p.name==='string'&&typeof p.data==='string'&&/^data:image\/(jpeg|png|webp);base64,[A-Za-z0-9+/=\r\n]+$/.test(p.data)&&p.data.length<5700000;}
...
async function init(){paintIcons();try{db=await openDatabase();db.onversionchange=()=>{db.close();toast('تم تحديث التخزين. أعد تحميل الصفحة.');};await refreshData();bindEvents();route();}catch(error){$('#view-content').innerHTML=`<div class="error-panel"><h2>تعذر فتح التخزين المحلي</h2><p>اسمح بتخزين بيانات المواقع واستخدم نافذة عادية بدل التصفح الخاص، ثم أعد تحميل الصفحة. لن ندّعي حفظ أي بيانات عندما يكون التخزين غير متاح.</p><p>${esc(error.message)}</p></div>`;$('#storage-status').textContent='التخزين غير متاح';$('#add-income').disabled=true;$('#add-expense').disabled=true;}}
init();
